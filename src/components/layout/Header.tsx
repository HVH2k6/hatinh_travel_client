      ...
      <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between gap-4 px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/" aria-label="Trang chủ" className="inline-flex items-center gap-2">
            <Image src="/logo.png" alt="logo" width={40} height={40} priority className="size-10 rounded-md object-cover" />
          </Link>
        ...
        {/* Center: search */}
        <form role="search" onSubmit={onSearch} className="hidden w-full max-w-xl items-center gap-2 md:flex">
          <div className="relative flex-1">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 opacity-60" />
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm kiếm địa điểm, đặc sản..." className="pl-9 bg-slate-50 focus:bg-white" />
          </div>
        </form>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center">
             {isLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin text-blue-600 hidden sm:block" />}
          ...
          <form onSubmit={onSearch} className="md:hidden">
            <div className="relative">
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm..." className="w-[100px] pl-8 h-9 text-xs" />
          ...
          {loading ? (
            ...
            <Button asChild variant="default" size="sm" className="hidden sm:inline-flex min-w-[96px] bg-blue-600 hover:bg-blue-700 text-white">
