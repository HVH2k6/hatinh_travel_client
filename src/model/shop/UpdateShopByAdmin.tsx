...
import { getProvinces, STATUS_SHOP } from '@/util/constant';
...
const formSchema = z.object({
  ...
  contact: z
    ...
    .optional(),
  status: z.enum(
    [STATUS_SHOP.ACTIVE, STATUS_SHOP.PENDING, STATUS_SHOP.DELETED],
    {
...
export default function UpdateShopByAdmin({ data }: Props) {
  ...
  const listStatus = useMemo(
    () => [
      { value: STATUS_SHOP.ACTIVE, label: 'Hoạt động' },
      { value: STATUS_SHOP.PENDING, label: 'Khóa tạm thời' },
      { value: STATUS_SHOP.DELETED, label: 'Khóa vĩnh viễn' },
    ],
    []
  );
  ...
  const form = useForm<FormType>({
    ...
    defaultValues: {
      ...
      status: data.status || STATUS_SHOP.ACTIVE,
      contact: {
