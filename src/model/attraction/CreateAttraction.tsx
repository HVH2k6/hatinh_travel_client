...
export const formSchema = z.object({
  ...
  list_image: z.array(z.string().url({ message: 'Mỗi hình ảnh phải là một URL hợp lệ.' })).min(1, { message: 'Cần ít nhất một hình ảnh mô tả.' }),
  ...
  isFree: z.boolean(),
  minPrice: z.number({ required_error: 'Vui lòng nhập giá tối thiểu.', invalid_type_error: 'Giá tối thiểu phải là số.' }).nonnegative({ message: 'Giá tối thiểu không được âm.' }),
  maxPrice: z.number({ required_error: 'Vui lòng nhập giá tối đa.', invalid_type_error: 'Giá tối đa phải là số.' }).nonnegative({ message: 'Giá tối đa không được âm.' }),
...
const CreateAttraction  = () => {
  ...
  // Load tỉnh và set tỉnh mặc định
  useEffect(() => {
